    ...
    .then((doc) => {
      if (doc.exists) {
        document.getElementById("studentName").innerHTML =
          "Hello " + doc.data().Name + "👋<br>Welcome to your Gradebook!";
...
function restoreValuesOnLogin(coursesData) {
  for (const key in coursesData) {
    if (Object.hasOwnProperty.call(coursesData, key)) {
      ...
      document.getElementById(
        `name_course${key}`
      ).value = `${courseDetails["NameCourse"]}`;
      document.getElementById(
        `credits${key}`
...
function viewCourses(key) {
  ...
  var yearSelect = createSelect("year" + key, "Year", [
    "1",
    "2",
    "3",
    "4",
  ...
  var semesterSelect = createSelect("semester" + key, "Semester", [
    "A",
    "B",
    "S",
  ]);
...
function addNewCourse() {
  if (idInUse.length < 100) {
    ...
    var yearSelect = createSelect("year" + uniqueRandomId, "Year", [
      "1",
      "2",
      "3",
      "4",
    ...
    var semesterSelect = createSelect("semester" + uniqueRandomId, "Semester", [
      "A",
      "B",
      "S",
    ]);
    var nameInput = createInput(
      "text",
      "name_course" + uniqueRandomId,
      "Name course",
      "Name course"
    );
    var creditsInput = createInput(
      "number",
      "credits" + uniqueRandomId,
      "Credits",
      ""
    );
    var gradeInput = createInput(
      "number",
      "grade" + uniqueRandomId,
      "Final grade",
      ""
    );
    var deleteButton = createButton(
      "deleteCourse(this, '" + uniqueRandomId + "')",
      "Delete",
      "20%",
      "fa fa-trash"
    );
...
function deleteCourse(item, idCourse) {
  ...
  docRef.get().then((doc) => {
    var nameCourseDeleted;
    if (doc.data().hasOwnProperty("Courses")) {
      if (doc.data().Courses[idCourse]) {
        nameCourseDeleted = doc.data().Courses[idCourse].NameCourse;
      } else {
        nameCourseDeleted = null;
      }
      docRef
        .update({
        ...
        .then(() => {
          ...
          if (nameCourseDeleted != null) {
            showLoader(nameCourseDeleted);
          }
        })
        .catch((error) => {
...
function showLoader(flag) {
  return new Promise(function (resolve) {
    ...
    if (flag == "save") {
      message.innerHTML =
        "Courses saved successfully!<br><i class='fa fa-check' aria-hidden='true'></i>";
    } else {
      message.innerHTML = `${flag} course successfully deleted!<br><i class='fa fa-check' aria-hidden='true'></i>`;
    }
    overlay.style.display = "block";
    loader.style.display = "block";
